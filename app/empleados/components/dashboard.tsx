...
export default function EmpleadosDashboard({
  ...
  const currencyFormatter = useMemo(() => new Intl.NumberFormat("es-AR", { style: "currency", currency: "ARS", maximumFractionDigits: 0 }), []);
  const currency = (value: any) => (value == null ? "—" : currencyFormatter.format(value));
  ...
  useEffect(() => {
    try {
      ...
      const formatted = currencyFormatter.format(12345);
      console.assert(typeof formatted === "string" && /\$|ARS/.test(formatted), "currency debería formatear en ARS");
      console.assert(Array.isArray(openClaims), "openClaims debe ser un array");
    } catch (error) {
      console.error(error);
    }
  }, [currencyFormatter, openClaims]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-emerald-100 p-4 md:p-8">
      <div className="max-w-7xl mx-auto flex gap-6">
        <aside
          id="sidebar"
          className={`transition-all duration-300 ${sidebarVisible ? (sidebarCollapsed ? "w-16" : "w-64") : "w-0"} ${sidebarVisible ? "block" : "hidden"} xl:block`}
        >
          <nav className="bg-white/80 backdrop-blur-xl border border-emerald-100 rounded-2xl p-4 md:p-5 shadow-sm h-full flex flex-col justify-between">
            <ul className="space-y-2 text-sm">
              <li>
                <button className="w-full text-left px-3 py-2 rounded-lg hover:bg-emerald-50">{sidebarCollapsed ? "MC" : "Mi cuenta"}</button>
              </li>
              <li>
                <button className="w-full text-left px-3 py-2 rounded-lg hover:bg-emerald-50">{sidebarCollapsed ? "Doc" : "Documentación"}</button>
              </li>
              <li>
                <button className="w-full text-left px-3 py-2 rounded-lg hover:bg-emerald-50">{sidebarCollapsed ? "PT" : "Próximos turnos"}</button>
              </li>
              <li>
                <button className="w-full text-left px-3 py-2 rounded-lg hover:bg-emerald-50">{sidebarCollapsed ? "OS" : "Oportunidades"}</button>
              </li>
              <li>
                <button className="w-full text-left px-3 py-2 rounded-lg hover:bg-emerald-50">{sidebarCollapsed ? "RP" : "Reputación"}</button>
              </li>
              <li>
            ...
            <div>
              <button onClick={() => setSidebarCollapsed((state) => !state)} className="w-full text-left px-3 py-2 rounded-lg border border-gray-200 hover:bg-gray-50">
                {sidebarCollapsed ? "»" : "«"} {sidebarCollapsed ? "Expandir" : "Colapsar"}
              </button>
            </div>
          </nav>
        </aside>
        ...
        <main className="flex-1">
          ...
          <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
            <div className="flex items-center gap-3">
              <button onClick={() => setSidebarCollapsed((state) => !state)} className="rounded-md p-1 bg-white/60 hover:bg-white hidden xl:inline" aria-label="Alternar ancho del menú">
                {sidebarCollapsed ? "»" : "«"}
              </button>
              <button
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-semibold text-gray-900">
                  Hola, {user?.role === "trabajador" ? user?.name?.split(" ")[0] : user?.name}
                </h1>
                <p className="text-sm text-gray-500">Tu actividad y oportunidades</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => onExport?.()} className="rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm hover:bg-gray-50">
                Descargar historial
              </button>
            </div>
          </header>
          ...
          {/* Próximos turnos + Balance / Reclamos */}
            ...
            <div className="xl:col-span-2 bg-white/80 backdrop-blur-xl border border-emerald-100 rounded-2xl p-4 md:p-5 shadow-sm">
              ...
              <div className="divide-y divide-gray-100 max-h-72 overflow-y-auto pr-1">
                {upcomingShifts.map((shift: any) => (
                  <div key={shift.id} className="py-3 flex items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-medium text-gray-900">
                        {shift.role} · <span className="text-gray-600">{shift.site}</span>
                      </p>
                    ...
                    <div className="flex items-center gap-2">
                      {shift.status === "Confirmado" ? (
                        <>
                          <button onClick={() => onCheckIn?.(shift)} className="rounded-xl bg-emerald-600 px-3 py-1.5 text-sm text-white hover:bg-emerald-700">
                            Check-in
                          </button>
                          <button onClick={() => onCheckOut?.(shift)} className="rounded-xl border border-gray-300 bg-white px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50">
                            Check-out
                          </button>
                        </>
                ...
                {upcomingShifts.length === 0 && <p className="py-6 text-center text-sm text-gray-500">No tenés turnos agendados.</p>}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="bg-white/80 backdrop-blur-xl border border-emerald-100 rounded-2xl p-4 md:p-5 shadow-sm">
                ...
                <div className="mt-3 text-right">
                  <button onClick={() => onExport?.()} className="text-sm rounded-xl border border-gray-300 bg-white px-3 py-2 hover:bg-gray-50">
                    Ver detalle
                  </button>
                </div>
              </div>
              ...
              <div className="bg-white/80 backdrop-blur-xl border border-emerald-100 rounded-2xl p-4 md:p-5 shadow-sm">
                ...
                <ul className="mt-2 space-y-2 text-sm">
                  {openClaims.map((claim) => (
                    <li key={claim.id} className="border-t border-gray-100 pt-2">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-gray-900">{claim.categoria}</span>
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full ${claim.estado === "Abierto" ? "bg-amber-100 text-amber-800" : "bg-sky-100 text-sky-800"}`}
                        >
                          {claim.estado}
                        </span>
                      </div>
                      <div className="text-gray-600">{claim.resumen}</div>
                      <div className="text-xs text-gray-500">
                        {claim.id} · {claim.severidad} · {claim.creado}
                      </div>
                      <div className="mt-2 text-right">
                        <button
                          onClick={() => (typeof onViewClaim === "function" ? onViewClaim(claim) : (window.location.href = `/reclamos/${encodeURIComponent(claim.id)}`))}
                  ...
                  {openClaims.length === 0 && <li className="text-gray-500">No tenés reclamos abiertos.</li>}
                </ul>
              </div>
            </div>
          </section>
          ...
          <section className="grid grid-cols-1 xl:grid-cols-3 gap-4 mt-4">
                  ...
                  <tbody>
                    {suggestedJobs.map((job: any) => (
                      <tr key={job.id} className="border-t border-gray-100">
                        <td className="py-2 pr-3 font-medium text-gray-900">{job.role}</td>
                        <td className="py-2 pr-3">{job.site}</td>
                        <td className="py-2 pr-3">{formatDate(job.date)}</td>
                        <td className="py-2 pr-3">{job.time}</td>
                        <td className="py-2 pr-3">{job.distanceKm} km</td>
                        <td className="py-2 pr-3">{currency(job.pay)}</td>
                        <td className="py-2">
                          <div className="flex gap-2 justify-end">
                            <button onClick={() => handleViewDetails(job)} className="rounded-lg border border-gray-300 bg-white px-2.5 py-1.5 text-gray-700 hover:bg-gray-50">
                              Ver detalles
                            </button>
                            <button onClick={() => handleAcceptJob(job)} className="rounded-lg bg-emerald-600 text-white px-2.5 py-1.5 hover:bg-emerald-700">
                              Aceptar
                            </button>
                            <button onClick={() => handleRejectJob(job)} className="rounded-lg border border-gray-300 bg-white px-2.5 py-1.5 hover:bg-gray-50">
                              Rechazar
                            </button>
                          </div>
                        </td>
            ...
            <div className="space-y-4">
              <div className="bg-white/80 backdrop-blur-xl border border-emerald-100 rounded-2xl p-4 md:p-5 shadow-sm">
                    ...
                    <li key={document.id} className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full ${
                          document.status.includes("Vence")
                ...
                <div className="mt-3 text-right">
                  <button onClick={() => onUpdateDocs?.()} className="text-sm rounded-xl border border-gray-300 bg-white px-3 py-2 hover:bg-gray-50">
              ...
              <div className="bg-white/80 backdrop-blur-xl border border-emerald-100 rounded-2xl p-4 md:p-5 shadow-sm">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-gray-600">
                    Puntualidad {reputation.punctuality}% · Finalización {reputation.completion}%
                  </span>
                </div>
                <ul className="mt-3 space-y-2 text-sm text-gray-700">
                  {reputation.reviews.map((review: any) => (
                    <li key={review.id} className="border-t border-gray-100 pt-2">
                      <strong className="text-gray-900">{review.by}:</strong> {review.text}
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div role="dialog" aria-modal="true" className="relative z-10 w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <div className="flex items-start justify-between gap-4">
              ...
              <button onClick={handleCloseModal} className="rounded-xl border border-gray-200 px-2 py-1 text-sm text-gray-600 hover:bg-gray-50" aria-label="Cerrar">
                Cerrar
              </button>
            </div>
            <div className="mt-4 space-y-2 text-sm text-gray-700">
              <p>
                <span className="font-medium text-gray-600">Fecha:</span> {formatDate(selectedJob.date)}
              </p>
              <p>
                <span className="font-medium text-gray-600">Horario:</span> {selectedJob.time}
              </p>
              {selectedJob.distanceKm !== undefined && (
                <p>
                  <span className="font-medium text-gray-600">Distancia estimada:</span> {selectedJob.distanceKm} km
                </p>
              )}
              <p>
                <span className="font-medium text-gray-600">Pago:</span> {currency(selectedJob.pay)}
              </p>
            </div>
            <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:justify-end">
              <button onClick={() => handleRejectJob(selectedJob)} className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 hover:bg-gray-50">
                Rechazar
              </button>
              <button onClick={() => handleAcceptJob(selectedJob)} className="rounded-lg bg-emerald-600 px-3 py-2 text-sm text-white hover:bg-emerald-700">
                Aceptar
              </button>
            </div>
          </div>
